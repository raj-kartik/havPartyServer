...
export const getAllEvents = async (req, res) => {
  ...
  try {
    ...
    if (ownerId) {
      const owner = await Owner.findById(ownerId).populate('clubs_owned', '_id name');
      if (!owner || !owner.clubs_owned.length) {
        return res.status(404).json({ message: 'Owner not found or no clubs found' });
      }
      clubIds = owner.clubs_owned.map((club) => club._id);
    }

    if (partnerId) {
      const employee = await Employee.findById(partnerId).populate('club', '_id name position');
      if (!employee || !employee.club) {
        return res.status(404).json({ message: 'Employee not found or not assigned to a club' });
      }

      if (employee.position !== 'Manager') {
        return res.status(403).json({ message: 'Only Managers can fetch events' });
      }
    ...
    const events = await Event.find({ clubId: { $in: clubIds } }).populate('clubId', 'name');
...
export const postCreateEvent = async (req, res) => {
  try {
    ...
    if (createdByModel === "Owner") {
      ...
      if (!owner) return res.status(400).json({ message: "Invalid owner ID" });
