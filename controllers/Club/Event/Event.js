...
export const createEvent = async (req, res) => {
  const { clubId, title, description, instructions, photos, dates, timings } =
    req.body;
  ...
  try {
    ...
    if (!dates || !Array.isArray(dates) || dates.length === 0) {
      return res
        .status(400)
        .json({ message: "At least one event date is required" });
    }
    if (!timings || !Array.isArray(timings) || timings.length === 0) {
      return res
        .status(400)
        .json({ message: "At least one event timing is required" });
    }
